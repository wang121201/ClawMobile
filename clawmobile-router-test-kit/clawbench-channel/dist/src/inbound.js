...
function compactJsonValue(value, maxTextChars, depth = 0) {
    if (value === undefined) {
        return null;
    }
...
function summarizeToolResult(content, maxTextChars) {
...
function compactMessagesSnapshot(messages, maxTextChars) {
...
function resolveAdjacentTrajectoryFilePath(sessionFile) {
    return sessionFile.endsWith(".jsonl")
        ? `${sessionFile.slice(0, -".jsonl".length)}.trajectory.jsonl`
        : `${sessionFile}.trajectory.jsonl`;
}
function readPointerRuntimeFile(sessionFile) {
    try {
        const pointer = JSON.parse(fs.readFileSync(resolveTrajectoryPointerFilePath(sessionFile), "utf8"));
        if (!pointer || typeof pointer !== "object" || Array.isArray(pointer)) {
            return undefined;
        }
        const runtimeFile = pointer.runtimeFile;
...
function resolveRuntimeTrajectoryFile(sessionFile) {
    const candidates = [
...
function readRuntimeTrajectorySnapshot(params) {
    try {
        ...
        const maxBytes = positiveIntegerEnv("CLAWBENCH_RUNTIME_TRAJECTORY_MAX_BYTES", DEFAULT_RUNTIME_TRAJECTORY_MAX_BYTES);
        const maxSteps = positiveIntegerEnv("CLAWBENCH_RUNTIME_TRAJECTORY_MAX_STEPS", DEFAULT_RUNTIME_TRAJECTORY_MAX_STEPS);
        const maxTextChars = positiveIntegerEnv("CLAWBENCH_RUNTIME_TRAJECTORY_TEXT_MAX_CHARS", DEFAULT_RUNTIME_TRAJECTORY_TEXT_MAX_CHARS);
        ...
        const parsedEvents = lines.flatMap((line) => {
            try {
