...
import { completeClawBenchRun, createClawBenchRun, failClawBenchRun, getClawBenchRunSnapshot, markClawBenchRunRunning, } from "./runs.js";
...
async function handlePostRun(params, req) {
    ...
    queueMicrotask(() => {
        ...
        handleClawBenchInbound({
            ...
            params.log?.error?.(`[${params.account.accountId}] clawbench run ${runId} failed: ${message}`);
            failClawBenchRun({
                accountId: params.account.accountId,
                runId,
                error: message,
...
async function requestHandler(params, req, res) {
    try {
...
export async function startClawBenchHttpServer(params) {
    ...
    params.log?.info?.(`[${params.account.accountId}] clawbench channel listening on ${params.account.baseUrl}`);
